...
export async function login(req, res) {
  ...
  try {
    const response = await database.oneOrNone(
      "SELECT cargo, senha FROM pessoa WHERE cpf = $1",
      user.cpf
    );
    ...
    if (valida === false) {
      ...
      const token = criarToken(user.cpf, response.cargo);
...
export async function cadastro(req, res) {
  ...
  if (!senha || senha.length < 6) {
    return res
      .status(400)
      .json({ mensagem: "A senha deve conter pelo menos 6 caracteres!" });
  }
  ...
  try {
    await database.none(
      "INSERT INTO pessoa (cpf, nome, senha, endereco) VALUES ($1, $2, $3, $4);",
      [cpf, nome, hashedPassword, endereco]
    );

    const token = criarToken(cpf);

    res.status(201).json({ mensagem: "Cadastrado com sucesso", token, cpf });
  } catch (error) {
    console.error("mensagem ", error);
...
export async function atualizar(req, res) {
  ...
  try {
    ...
    // atualiza a pessoa
    await database.none(
      "UPDATE pessoa SET nome = $1, endereco = $2, cargo = $3, senha = $4 WHERE cpf = $5;",
      [nome, endereco, cargo, senha, cpf]
    );
