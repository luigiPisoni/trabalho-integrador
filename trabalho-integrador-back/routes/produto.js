...
produtoRouter.post('/novo', async (req, res) => {
  ...
  if (parseFloat(valor) < 0) {
    return res.status(400).send({
      mensagem: 'O valor não pode ser negativo seu jumento vai pagar pra comprar',
    });
  }
...
produtoRouter.delete('/deletar/:codprt', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ mensagem: 'Erro no servidor ao tentar deletar o produto' });
  }
});

produtoRouter.put('/atualizar/:codpdt', async (req, res) => {
  const { codpdt } = req.params;
  var { nome, valor } = req.body;

  if (valor && parseFloat(valor) < 0) {
    return res.status(400).send({ mensagem: 'O valor não pode ser negativo mano' });
  }

  try {
    const produtoExistente = await database.oneOrNone('SELECT nome, valor FROM produto WHERE codpdt = $1;', [codpdt]);
    ...
    await database.none('UPDATE produto SET nome = $1, valor = $2 WHERE codpdt = $3;', [nome, valor, codpdt]);

    res.status(200).json({ mensagem: 'Produto atualizado com sucesso' });
  } catch (error) {
    console.error('Erro ao atualizar produto: ', error);
    res.status(500).json({ mensagem: 'Erro no servidor ao tentar atualizar o produto' });
  }
});
